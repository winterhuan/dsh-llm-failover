...
Each group needs at least two distinct `{ provider, model }` targets. A target's non-negative `retryCount` is the number of additional attempts on that same route after its first eligible failure; omission defaults to `0`, preserving immediate failover. The conversation model selector lists every group under the **模型组** provider, and that choice applies to the selected session. `activeGroup` is a global switch: once set, new requests in every session follow the group regardless of the session's own model choice, until you change or clear it; omitting it leaves normal model selection unchanged. The default eligible codes are `EMPTY_RESPONSE`, `RATE_LIMIT`, `SERVER`, `TIMEOUT`, and `TRANSPORT`.
...
The interaction with the base bundle is concrete: base always mounts `llm-retry`, and an installed plugin's composition row can only be appended after it, so `llm-retry`'s listener runs upstream of this plugin. In `normal` mode it first spends the target route's own provider-level retry budget (the adapter profile default is 5) before this plugin decides, so a target's `retryCount` stacks on top of those attempts — a dead target can survive several backoff cycles before the group advances it. The page's retry-budget notice offers a one-click write of each referenced route's `retryPolicy.maxRetries` to `0`, making an eligible failure switch immediately; that write affects every use of the route and stays reversible in the advanced editors.
...
The settings page loads provider and model catalogs from the Host. The provider dropdown includes only currently active routes, the model dropdown follows the selected provider, and each target has an editable retry count. Retryable errors use a multi-select dropdown; when omitted, the default set is displayed and selected. The page also supports adding groups, choosing the active group, and arranging targets with the up/down buttons beside each row. The header shows an unsaved-changes chip, saves are gated by inline validation against the host rules (including a dangling active-group reference), and a Discard control restores the last saved snapshot — group drafts and advanced drafts together. Save uses the host Settings revision, so a conflicting external edit is rejected rather than overwritten. Controls come from the host's shared primitives (Button, Input, Pill, and the `--dsw-alias-*` tokens), matching the native Models settings page.

When a referenced target route still keeps its own provider-level automatic retries, a retry-budget notice above the actions row lists those routes and explains that they run before the group switches. Each route gets a one-click "set retries to 0" write through the same `settings.mutate` channel and conflict handling, and the whole notice can be dismissed.
